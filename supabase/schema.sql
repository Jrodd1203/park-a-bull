...
CREATE OR REPLACE FUNCTION update_lot_occupancy()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER update_occupancy_on_checkin AFTER INSERT OR UPDATE ON checkins
  FOR EACH ROW EXECUTE FUNCTION update_lot_occupancy();
...
CREATE TRIGGER update_profile_on_checkin AFTER INSERT OR UPDATE ON checkins
  FOR EACH ROW EXECUTE FUNCTION increment_checkin_count();
